...
export default async function AdminUserDetailPage({ params }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="border-bordurewarm-tertiary dark:border-encre/20 rounded-2xl border bg-white p-5">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="border-bordurewarm-tertiary dark:border-encre/20 rounded-2xl border bg-white p-5">
          <h3 className="text-encre dark:text-creme mb-2 text-sm font-semibold">
            Annonces
          </h3>
          ...
          <ul className="space-y-1 text-sm">
            {(audit ?? []).map((a) => (
              <li key={a.id} className="flex justify-between">
                <span>{a.action}</span>
                <span className="text-encre/70 dark:text-creme/70 text-xs">

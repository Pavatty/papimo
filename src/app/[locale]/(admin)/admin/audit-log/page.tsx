...
export default async function AdminAuditLogPage({
  ...
  return (
    <div className="space-y-4">
      <h1 className="font-display text-encre text-2xl font-bold">Audit log</h1>
      ...
      <div className="border-bordurewarm-tertiary overflow-auto rounded-xl border bg-white">
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody>
            {(rows ?? []).map((row) => (
              <tr key={row.id} className="border-bordurewarm-tertiary border-t">
                <td className="px-3 py-2">
                  {new Date(row.created_at).toLocaleString("fr-FR")}
                </td>
                <td className="px-3 py-2">{row.action}</td>
                <td className="px-3 py-2">{row.target_type ?? "-"}</td>
                <td className="text-encre/70 px-3 py-2 text-xs">
                  {row.target_id ?? "-"}
                </td>
                <td className="text-encre/70 px-3 py-2 text-xs">
                  {row.admin_id ?? "-"}
